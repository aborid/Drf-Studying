...
class AddressGenericAPIView(GenericAPIView,CreateModelMixin,RetrieveModelMixin,UpdateModelMixin,DestroyModelMixin):
    ...
    def post(self,request):
        ...
        #     如果这个值是true，那么这个地址就是默认地址，我需要把所有的地址先改成0
    ...
    def get(self,request):
        if not request.user.get("status"):
            return JsonResponse(request.user,safe=False)
        email=request.user.get("data").get("username")
        db_request = self.get_queryset().filter(email=email).all().order_by("-default","create_time")
        ser=self.get_serializer(instance=db_request,many=True)
...
class UserAddressDetailGenericAPIView(GenericAPIView,ListModelMixin,CreateModelMixin,RetrieveModelMixin,UpdateModelMixin,DestroyModelMixin):
    queryset = UserAddress.objects
    serializer_class = AddressSerializer
